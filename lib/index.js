export default class AggregationBuilder {
    constructor(model) {
        ...
        this.lookup = function (arg, options) {
            ...
            if (arg.let) {
                /**
                 * Correlated Subquery Lookup (with let variables)
                 * @see https://www.mongodb.com/docs/manual/reference/operator/aggregation/lookup/#join-conditions-and-subqueries
                 */
                stage = { $lookup: {} };
                if (!arg.pipeline)
                    throw "key 'pipeline' is required when using 'let' in lookup aggregation stage";
                stage.$lookup.from = arg.from;
                stage.$lookup.let = arg.let;
                stage.$lookup.pipeline = arg.pipeline;
                stage.$lookup.as = arg.as || arg.from;
                // MongoDB 5.0+ supports localField/foreignField with let/pipeline
                if (arg.localField)
            ...
            else if (arg.localField) {
                /**
                 * Equality Match Lookup (optionally with pipeline for MongoDB 5.0+)
                 * @see https://www.mongodb.com/docs/manual/reference/operator/aggregation/lookup/#equality-match
                 */
                stage = { $lookup: {} };
                stage.$lookup.from = arg.from;
                stage.$lookup.localField = arg.localField;
                ...
                // MongoDB 5.0+ supports adding pipeline to equality match
                if (arg.pipeline)
                    stage.$lookup.pipeline = arg.pipeline;
                this.closeStage(stage);
            }
                ...
                /**
                 * Uncorrelated Subquery Lookup (pipeline only, no let)
                 * @see https://www.mongodb.com/docs/manual/reference/operator/aggregation/lookup/#uncorrelated-subquery
                 */
                stage = { $lookup: {} };
                stage.$lookup.from = arg.from;
                stage.$lookup.pipeline = arg.pipeline;
                ...
                throw "lookup requires either 'localField' or 'pipeline' to be specified";
            }
         ...
         * @return this stage
         */
        this.unionWith = function (arg, options) {
            if (!this.openStage("lookup", options))
                return this;
            if (!arg.pipeline)
         ...
         * @return this stage
         */
        this.unset = function (arg, options) {
            if (!this.openStage("unset", options))
                return this;
            /**
             *
             */
         ...
         * @return this stage
         */
        this.count = function (string, options) {
            if (!this.openStage("count", options))
                return this;
         ...
         * @return this stage
         */
        this.geoNear = function (arg, options) {
            if (!this.openStage("geoNear", options))
                return this;
            if (this.aggs?.length) {
                throw new Error("The $geoNear stage must be the first stage in the aggregation pipeline.");
            }
            /**
             * @see GeoNear
             */
            ...
            if (arg.spherical !== undefined)
                stage.$geoNear.spherical = arg.spherical;
         ...
         * @returns this stage
         */
        this.dateFromString = function (dateString, format, timezone, onNull, options) {
            try {
                const stage = {
                    $dateFromString: {
         ...
         *
         */
        this.convert = function (input, to, onError) {
            let data = {
                input: input,
                to: to,
            };
         ...
         * @returns this operator
         */
        this.concatArrays = function (arr1, arr2, arr3, arr4, arr5, arr6, arr7, arr8, arr9, arr10) {
            let arr = [arr1, arr2];
         ...
         * @returns this operator
         */
        this.setUnion = function (arr1, arr2, arr3, arr4, arr5, arr6, arr7, arr8, arr9, arr10) {
            let arr = [arr1, arr2];
            arr3 !== undefined ? arr.push(arr3) : 0;
            arr4 !== undefined ? arr.push(arr4) : 0;
            arr5 !== undefined ? arr.push(arr5) : 0;
         ...
         * @returns this operator
         */
        this.arrayToObject = function (expr) {
            try {
         ...
         * @returns this operator
         */
        this.anyElementTrue = function (expr) {
            try {
         ...
         * @returns this operator
         */
        this.setIntersection = function (expr) {
            try {
         ...
         *
         */
        this.regexMatch = function (input, regex, options) {
            try {
                const stage = {
                    $regexMatch: {
                        input: `$${input}`,
         ...
         * @return this stage
         */
        this.search = function (searchDoc) {
            if (!this.openStage("search"))
                return this;
            if (this.aggs?.length) {
                throw new Error("The $search stage must be the first stage in the aggregation pipeline.");
            }
         ...
         */
        this.searchCompound = function (arg) {
            if (!this.openStage("search"))
                return this;
            if (this.aggs?.length) {
                throw new Error("The $search stage must be the first stage in the aggregation pipeline.");
         ...
         */
        this.searchEquals = function ({ path, value, }) {
         ...
         */
        this.searchAutocomplete = function ({ path, query, tokenOrder, fuzzy, }) {
            return { autocomplete: { path, query, tokenOrder, fuzzy } };
        };
        this.searchShould = function (operator, minimumShouldMatch) {
            try {
                if (!this.openStage("search"))
                    return this;
                const latestStage = this.aggs[this.aggs.length - 1];
        ...
        this.searchMust = function (operator) {
            try {
                if (!this.openStage("search"))
                    return this;
                const latestStage = this.aggs[this.aggs.length - 1];
        ...
        this.searchMustNot = function (operator) {
            try {
                if (!this.openStage("search"))
                    return this;
                const latestStage = this.aggs[this.aggs.length - 1];
        ...
        this.searchFilter = function (operator) {
            try {
                if (!this.openStage("search"))
                    return this;
                const latestStage = this.aggs[this.aggs.length - 1];
         ...
         */
        this.vectorSearch = function (vectorSearchArg) {
            if (!this.openStage("vectorSearch"))
                return this;
            if (this.aggs?.length) {
                throw new Error("The $vectorSearch stage must be the first stage in the aggregation pipeline.");
            }
         ...
         *  }
         */
        this.sortArray = function (input, sortBy) {
            try {
